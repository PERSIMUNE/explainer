...
mydata$Cl.thickness <- as.numeric(as.character(mydata$Cl.thickness))
mydata$Cell.size <- as.numeric(as.character(mydata$Cell.size))
mydata$Cell.shape <- as.numeric(as.character(mydata$Cell.shape))
mydata$Marg.adhesion <- as.numeric(as.character(mydata$Marg.adhesion))
mydata$Epith.c.size <- as.numeric(as.character(mydata$Epith.c.size))
mydata$Bare.nuclei <- as.numeric(as.character(mydata$Bare.nuclei))
mydata$Bl.cromatin <- as.numeric(as.character(mydata$Bl.cromatin))
mydata$Normal.nucleoli <- as.numeric(as.character(mydata$Normal.nucleoli))
mydata$Mitoses <- as.numeric(as.character(mydata$Mitoses))
# mydata$Class <- ifelse(mydata$Class == "benign", TRUE, FALSE)
mydata$sex <- as.numeric(as.character(mydata$sex))
...
mylrn <- mlr3::lrn("classif.lightgbm", predict_type = "prob")
# train the model
