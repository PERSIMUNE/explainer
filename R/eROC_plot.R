...
eROC_plot <- function(task,
  ...
  bmr_PRroc_plt <- bmr_PRroc_plt +
    labs(
      title = paste0("development set N=", nrow(featset_total), ", PRAUC = ", round(prf_dev[prauc_idx, 1], 2)),
  ...
  bmr_roc_plt <- bmr_roc_plt +
    labs(
      title = paste0("development set N=", nrow(featset_total), ", AUC = ", round(prf_dev[auc_idx, 1], 2)),
  ...
  bmr_PRroc_plt_test <- bmr_PRroc_plt_test +
    labs(
      title = paste0("test set N=", nrow(featset_total_test), ", PRAUC = ", round(prf_test[auc_idx, 1], 2)),
      x = "Recall", y = "Precision"
    ) +
    egg::theme_article() +
    theme(legend.position = c(0.8, 0.9), legend.title = element_blank()) +
  ...
  bmr_roc_plt_test <- bmr_roc_plt_test +
    labs(
      title = paste0("test set N=", nrow(featset_total_test), ", AUC = ", round(prf_test[prauc_idx, 1], 2)),
      x = "False Positive Rate (FPR)", y = "True Positive Rate (TPR)"
    ) +
    egg::theme_article() +
    theme(legend.position = c(0.8, 0.1), legend.title = element_blank()) +
