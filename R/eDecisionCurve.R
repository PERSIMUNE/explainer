...
eDecisionCurve <- function(task,
                           ...
                           splits,
                           seed = 246) {
  # utils::globalVariables(c("threshold", "value", "variable"))
  threshold <- NULL
  value <- NULL
  variable <- NULL
  # library(ggplot2)
  ...
  # to have the same format as mlr3 column names
  positive_class_label <- task$positive
  negative_class_label <- task$negative
  outcome <- task$target_names
  poscol <- paste0("prob.",task$positive) # column of the positive class
  ...
  for (i in seq(0.01,0.99,.01)){
    fpos[x] = sum(featset_total_test[,outcome]==negative_class_label & pred_results[,poscol]>=i, na.rm = T)
    Tpos[x] = sum(featset_total_test[,outcome]==positive_class_label & pred_results[,poscol]>=i, na.rm = T)
    # ref: https://en.wikipedia.org/wiki/Decision_curve_analysis and https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2577036/
  ...
  decision_curve_plt <- ggplot(data = netbenefit_df_long, aes(x=threshold, y=value, colour= variable)) +
    ...
    geom_vline(xintercept = 0.5, colour="blue", linetype = "longdash") + # the vertical line
